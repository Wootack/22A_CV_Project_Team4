...
def return_pose(image , image2 , keeper , referee):
	...
	for i in range(len(person_conf_multi)):
		...
		if len(allXCoords) == 4:
			...
			if len(teamClassificationFeatures)!=0:
				jump = 1
			...
			if len(teamClassificationFeaturesDBSCAN)!=0:
				jump = 1
...
#  [] empty list error
	# print("Maybe error in here:", allTeamClassificationFeatures)
	# print(len(allTeamClassificationFeatures))
	# print(len(allTeamClassificationFeatures[0]))
	teamLabels = teamClassifier.fit_predict(allTeamClassificationFeatures)
	# print("Maybe error in here:", allTeamClassificationFeaturesDBSCAN)
